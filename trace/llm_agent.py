...
class OpenAIBackend(BaseLLMBackend):
    ...
    def make_tool_result_messages(
        ...
        # OpenAI expects separate messages, but we pack them as a list trick:
        # We return the first one; the caller must handle appending all.
        # Actually, let's return assistant + a combined list as second element.
        tool_msgs = []
        for tc, result_str in zip(tool_calls, results):
            tool_msgs.append({
                "role": "tool",
        ...
        # Return assistant_msg and a special wrapper that the agent loop unpacks
        return assistant_msg, tool_msgs  # type: ignore[return-value]
...
connections, and execute placements. Use them to make informed decisions.
...
Work step by step: check status, look at candidates, reason about flow, then place."""
...
class ExplorerAgent:
    ...
    def _execute(self, tc: ToolCall, explorer: Any) -> str:
        ...
        try:
            ...
            if name == "status":
                return _tool_status(explorer)
            elif name == "candidates":
                return _tool_candidates(explorer, top_k=args.get("top_k", 8))
            elif name == "detail":
                return _tool_detail(explorer, node_id=args.get("node_id"))
            elif name == "flow_info":
                gid = args.get("gid", "")
                if not gid:
                    return "Error: gid is required."
                return _tool_flow_info(explorer, gid=gid)
            elif name == "agent_recommend":
                return _tool_agent_recommend(explorer)
            elif name == "place":
                idx = args.get("candidate_index")
                if idx is None:
                    return "Error: candidate_index is required."
                return _tool_place(explorer, candidate_index=int(idx))
            elif name == "undo":
                return _tool_undo(explorer)
            elif name == "done":
